...
# --- Optional Vanna router integration ---------------------------------------
try:
    from vanna_router import router as vanna_router
except Exception:
    vanna_router = None

try:
    from vanna_router import auto_connect_from_env
except Exception:
    auto_connect_from_env = None

# -----------------------------------------------------------------------------
# App + CORS (fix: explicit origins, not "*", when credentials are used)
# -----------------------------------------------------------------------------
...
app.add_middleware(
    ...
    allow_credentials=True,          # ✅ ok now (no wildcard)
    allow_methods=["*"],
    allow_headers=["*"],
    expose_headers=["*"],
)

# Include Vanna API routes if present
if vanna_router is not None:
    app.include_router(vanna_router)

# -----------------------------------------------------------------------------
# Shared file upload (used by GenBI)
# -----------------------------------------------------------------------------
import unicodedata
import string
...
def _safe_filename(name: str) -> str:
    ...
    cleaned = "".join(
        c for c in unicodedata.normalize("NFKD", name) if c in keep
    ).strip().replace(" ", "_")
...
async def files_upload(file: UploadFile):
    ...
    mdl = {
        "catalog": "local",
        "schema": "public",
        "models": [{"name": Path(fname).stem, "refSql": ref_sql}],
    }
...
# -----------------------------------------------------------------------------
# Function index & discovery (pandas / numpy + synthetic iloc/loc)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Param coercion, indexer parsing, IO param normalization
# -----------------------------------------------------------------------------
            ...
            try:
                if is_indexer:
                    if recv is None:
                        raise HTTPException(
                            status_code=400,
                            detail=f"Node '{node_id}' ({func_name}) requires 'self' (a DataFrame/Series)",
                        )
...
async def nl2yaml(
    ...
    # If no keys configured, heuristic
    if not or_key and not ds_key:
        spec = _heuristic_nl_to_yaml(prompt)
        if not spec:
            raise HTTPException(
                status_code=400,
                detail="Heuristic NL→YAML couldn't understand the request. Configure OPENROUTER_API_KEY or DEEPSEEK_API_KEY for LLM mode.",
            )
    else:
        guideline = [
        ...
        if (receiver := (recv or "")):
            guideline.append(f"- Continue from the existing node {receiver}. For any DataFrame.* operation, set `self: {receiver}` and depend on {receiver}, unless the user explicitly names a different input.")
...
DATABASE_URL = os.getenv("DATABASE_URL", "postgresql://dappa:password@localhost:5432/dappa")
